...
`.github/vuln-scan/scripts/combine.py`'s module docstring for the authoritative schema), used to
develop and test the Choreo dashboard (Track B) against a stable contract before Track A's real
...
(in `choreo-dashboard/types.bal`) is a closed record, matching what `combine.py` actually emits.
